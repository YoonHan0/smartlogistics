...
const MyData = ({ info, setUserInfo }) => {
  ...
  const [userData, setUserData] = useState({ id: '', name: '', password: '', phone: '', profile: '' });
  const [messages, setMessages] = useState({ pwd: '', pwdchk: '', name: '', phone: '' });
  ...
  const onChangeHandle = (e) => {
    ...
    if (
      ...
      if (name === 'password') {
        ...
        setUserData((prev) => ({ ...prev, password: value.replace(/[^0-9a-zA-Z]/g, '') }));
  ...
  const validateForm = () => {
    ...
    // }
    if (userData.password && (userData.password.length < 9 || userData.password.length > 20)) {
      setMessages((prev) => ({ ...prev, pwd: '영어, 숫자를 포함한 최소 9자 최대 20자여야 합니다.' }));
    ...
    if (userData.password && userData.password !== pwdchk) {
      setMessages((prev) => ({ ...prev, pwdchk: '비밀번호가 일치하지 않습니다.' }));
    ...
    if (userData.phone === '') {
      ...
      setMessages((prev) => ({ ...prev, phone: '유효하지 않은 전화번호입니다.' }));
    } else {
    ...
    if (
      ...
      onSubmit(true);
      return;
    } else if (userData.id !== '' && userData.name !== '' && userData.phone !== '' && phoneRegex.test(userData.phone)) {
      onSubmit(true);
      return;
    }
  };
  const onSubmit = async (state) => {
    ...
    if (state === true) {
      ...
      await customFetch(`/api/user/updateprofile?uc=${info.user.sub}`, {
        ...
        .then((json) => {
          setUserData((prev) => ({ ...prev, id: info.user.sub, name: json.data.name, phone: json.data.phone }));
                ...
                <TableRow>
                  ...
                  <TableCell align="left" sx={{ display: 'flex' }}>
                    <TextField
                      ...
                      placeholder="영어,숫자 포함 최소 9자~최대12자"
                    />
                    {messages.pwd != '' && (
                      <Typography sx={{ width: '500px', color: 'red', marginLeft: 3 }}>{messages.pwd}</Typography>
                    )}
                  </TableCell>
                </TableRow>
                <TableRow>
                  ...
                  <TableCell align="left" sx={{ display: 'flex' }}>
                    ...
                    />
                    {messages.pwdchk != '' && (
                      <Typography sx={{ width: '500px', color: 'red', marginLeft: 3 }}>{messages.pwdchk}</Typography>
                    )}
                  </TableCell>
                </TableRow>
                <TableRow>
                  ...
                  <TableCell align="left" sx={{ display: 'flex' }}>
                    ...
                    />
                    {messages.name != '' && (
                      <Typography sx={{ width: '500px', color: 'red', marginLeft: 3 }}>{messages.name}</Typography>
                    )}
                  </TableCell>
                </TableRow>
                <TableRow>
                  <TableCell component="th" scope="row" sx={{ width: '200px', backgroundColor: '#F6F7F9' }}>
                    전화번호
                  </TableCell>
                  ...
                  <TableCell align="left" sx={{ display: 'flex' }}>
                    ...
                    />
                    {messages.phone != '' && (
                      <Typography sx={{ width: '500px', color: 'red', marginLeft: 3 }}>{messages.phone}</Typography>
                    )}
                  </TableCell>
                </TableRow>
              </TableBody>
